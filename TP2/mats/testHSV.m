...
% Azul3 -> 20 -> perfeito para 10, mean 0.30 mas no Azul4 tambem 0.30
% Azul4 -> 23 -> algumas coisinhas para 100, mean 0.30 com 0.52 no Azul3
...
%% colorThresholder
% [mask,res] = createMaskBrancov2(A,100);
% figure;
% imshow(mask)
% figure;
% imshow(res)
...
for idx = 1:6 
    ...
    mask= (H >= FundoLims(idx,1,1) & H <= FundoLims(idx,1,2)); %select by Hue
    mask=mask & (S >= FundoLims(idx,2,1) & S <= FundoLims(idx,2,2)); %add a condition for saturation
    mask=mask & (V >= FundoLims(idx,3,1) & V <= FundoLims(idx,3,2)); %add a condition for value
